...
void selection_SelectCurrentWord(EditView *ev)
{
	...
	ev->selection.anchor.y = ev->selection.extent.y = ev->cursor.y;
	ev->selection.anchor.line = ev->selection.extent.line = ev->curline;
	
	int x1, x2;
	ev->GetCurrentWordExtent(&x1, &x2);
	if (x2 < ev->curline->GetLength()) x2++;	// because extent is > than anchor
	ev->selection.anchor.x = x1;
	ev->selection.extent.x = x2;
