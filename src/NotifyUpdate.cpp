...
char line[1024];
	...
	snooze((int)(size_t)waittime);

	//stat("doing UC");

	// get temp file name
	char tmpfilename[MAXPATHLEN];
	find_directory(B_SYSTEM_TEMP_DIRECTORY, 0, true, tmpfilename, 
MAXPATHLEN-32);
...
void Go(uint delay)
{
	thread_id thread = spawn_thread(UpdateCheck::Thread, "UpdateCheck", B_LOW_PRIORITY, (void *)(size_t)delay);
